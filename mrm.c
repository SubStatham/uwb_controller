...
int mrmConfigGet(mrmConfiguration *config)
{
    ...
    {
        // Handle byte ordering(处理字节排序)
        confirm.msgType = ntohs(confirm.msgType);//ntos(network to host short)
        confirm.msgId = ntohs(confirm.msgId);

        // is this the correct message type?
        if (confirm.msgType == MRM_GET_CONFIG_CONFIRM)
        {
            // copy config from message to caller's structure(将配置从消息复制到调用方结构)
...
int mrmConnect(int connType, char *addr, mrm_uint32_t *connectionStatusCode)
{
    ...
    else
    {
        ...
        if (strncasecmp(addr, "COM", strlen("COM")) == 0)//比较字符串
...
int reboot(void)
{
	...
	// create request message
	request.msgType = htons(MRM_REBOOT_REQUEST);
	request.msgId = htons(msgIdCount++);
	...
	if (numBytes == sizeof(confirm))
	{
		...
		// is this the correct message type and is status good?
		if ((confirm.msgType == MRM_REBOOT_CONFIRM) &&
			(confirm.status == OK))
			retVal = OK;
	}
	return retVal;
