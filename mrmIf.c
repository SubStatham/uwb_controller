    ...
    switch (ifType)
    {
        ...
        case mrmIfUsb:
#ifdef WIN32
            {
				...
				//sprintf(comPortStr, "%s", destAddr);
				//printf(comPortStr);
                mbstowcs(wcs, comPortStr, sizeof(wcs));   //转换格式
...
static int mrmIfGetPacketIp(void *pkt, unsigned maxSize)
{
	...
	FD_ZERO(&fds);                 //
	FD_SET(radioFd, &fds);              //看不懂

	// Set up timeout
	tv.tv_sec = timeoutMs / 1000;
	tv.tv_usec = (timeoutMs * 1000) % 1000000;

	if (select(radioFd + 1, &fds, NULL, NULL, &tv) > 0)//防堵塞
	{
		// copy packet into buffer
		return recvfrom(radioFd, (char *)pkt, maxSize, 0, NULL, NULL);//接收文件
	}
